...
**Status:** Preliminary draft for AAEF v0.2.0 public review  
**AAEF version baseline:** v0.1.4 Public Review Draft  
**Scope:** Mapping from OWASP Agentic Top 10 risks to AAEF action assurance controls
...
- **Relevant AAEF Controls** — control requirements from `docs/en/07-control-requirements.md` and `controls/aaef-controls-v0.1.csv`.
- **Evidence Requirements** — evidence that should be captured for high-impact agentic actions.
- **v0.2 Notes** — gaps, clarifications, or candidate improvements for AAEF v0.2.0.
...
| OWASP ID | OWASP Risk | AAEF Perspective | Relevant AAEF Threats | Key AAEF Controls | v0.2 Notes |
|---|---|---|---|---|---|
...
| ASI04 | Agentic Supply Chain Vulnerabilities | AAEF does not replace supply chain security, but it requires inventory, operator accountability, tool classification, provenance, evidence, and isolation. | T1, T5, T6, T7, T9 | AAEF-GOV-01, AAEF-GOV-03, AAEF-ID-03, AAEF-TOOL-03, AAEF-MEM-02, AAEF-EVD-01, AAEF-RES-02, AAEF-RES-03 | Map to AI/agent supply chain frameworks in v0.2+. |
...
AAEF v0.2 should define **Principal Context Degradation** to address long-running tasks where the original principal intent becomes semantically distant from later actions.
...
AAEF v0.2 may define a **Tool Invocation Evidence Profile** for high-risk tools.
...
AAEF v0.2 should strengthen guidance for cross-domain principal binding and external agent identity verification.
...
AAEF v0.2 should refine how evidence events represent context influence, memory writes, memory reads, and retrieval provenance.
...
AAEF v0.2 should add a dedicated **Cross-Agent / Cross-Domain Authority** domain or control set.
...
AAEF v0.2 should add guidance for action sequences, dependency evidence, and cascade reconstruction.
...
AAEF v0.2 should expand approval quality controls, including approval threshold calibration, approval sampling, approval outcome monitoring, separation of duties, and approval bypass detection.
...
AAEF v0.2 should consider controls for agent admission, external agent trust decisions, and rogue agent quarantine.
...
### 4. v0.2 should prioritize A2A, evidence schema, and principal context

This mapping suggests that v0.2 should prioritize:
...
- and whether additional AAEF controls are needed for v0.2.
