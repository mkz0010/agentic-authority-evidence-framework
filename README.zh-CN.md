...
**AAEF v0.3.0 Public Review Draft** 是当前公开评审 baseline。

**AAEF v0.3.1 Maintenance Release** 是该 baseline 的最新已标记 maintenance release。

AAEF v0.3.1 不改变 v0.3.0 的公开评审范围。它增加了发布后清理、发布工件整理、版本引用一致性以及统一的 validation 自动化。

AAEF v0.3.0 在 v0.2.x baseline 的基础上增加了 implementation profiles、evidence quality assessment criteria、assessment profile mapping、TCB implementation capability patterns、action sequence monitoring、assessment automation guidance，以及 infrastructure / SIEM evidence integration guidance。

较早的 v0.2.x 和 v0.1.x 版本仍作为 prior public review baselines 保留。

AAEF v0.3.0 仍是公开评审草案。它不是认证机制、正式标准，也不声称能够完全缓解所有风险。
