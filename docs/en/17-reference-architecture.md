...
### Pattern E: Backend Relying Party Verification

In this pattern, the Agent Runtime and Tool Dispatch Enforcement Point may submit requests to a backend system, but the backend does not rely only on the runtime or dispatcher assertion.

The backend verifies that the request is supported by an action-bound authorization decision artifact, dispatch attestation, trusted principal identity, policy reference, or equivalent enforcement proof before applying the system effect.

This pattern is useful when the backend system is the final relying party for high-impact effects.

Risk:

- backend verification may be incomplete or inconsistently applied,
- authorization artifacts may be replayed if not bound to action, principal, time, and target,
- backend logs may not correlate cleanly with AAEF evidence events,
- operational teams may assume the dispatcher is sufficient and omit backend checks.

Recommended use:

- production systems where backend APIs perform material state changes,
- infrastructure, identity, finance, data, or security-sensitive actions,
- systems where direct backend calls must be rejected unless enforcement proof is present.

In this pattern, the backend API should reject high-impact requests that lack valid enforcement proof.

The backend should also generate or contribute to evidence records so the actual system effect can be correlated with the proposed action, authorization decision, dispatch event, and execution result.

### Pattern F: Audit-Grade Separated TCB

In this pattern, the trusted control boundary is separated from the Agent Runtime and model-controlled execution paths.

Authorization, dispatch enforcement, trusted runtime state, evidence generation, and evidence storage are operated as separately controlled components.

This is the strongest pattern described in this reference architecture.

Risk:

- implementation and operational complexity are higher,
- latency and cost may increase,
- teams may over-apply the pattern to low-risk use cases,
- weak integration discipline can still produce incomplete evidence chains.

Recommended use:

- audit-grade systems,
- regulated or externally reviewed environments,
- high-impact actions requiring post-incident reconstruction,
- systems where model/runtime compromise must not allow bypass of authorization, dispatch enforcement, or evidence generation.

In this pattern, model output remains only a proposed action.

A strong implementation should ensure that:

- policy evaluation is performed by a trusted Authorization Decision Point,
- high-impact credentials are unavailable to the Agent Runtime,
- dispatch is mediated by a Tool Dispatch Enforcement Point,
- backend systems verify action-bound enforcement proof where feasible,
- evidence is generated by trusted enforcement, backend, infrastructure, or evidence pipeline components,
- and evidence storage is protected against unauthorized modification or deletion.

### Capability Pattern Comparison

The following comparison summarizes how the deployment patterns relate to assessment profiles and trusted control placement.

| Pattern | Suitable profiles | Trusted boundary placement | TDE placement | Evidence expectation | Main limitation |
|---|---|---|---|---|---|
| Same-Process Prototype | Lite only, or limited Standard pilots | Weak or local process boundary | In-process wrapper or runtime code | Lightweight records; often self-reported unless strengthened | Model-influenced code may bypass authorization, dispatch enforcement, or evidence generation |
| Runtime-Enforced Agent Application | Standard, limited High-Impact with compensating controls | Trusted runtime or application middleware | Runtime-mediated tool invocation | Runtime-generated evidence with policy and dispatch records | Runtime compromise or bugs can affect both enforcement and evidence |
| External Authorization and Dispatch Gateway | Standard, High-Impact, some Audit-Grade | External authorization and dispatch components | Gateway or dedicated dispatch service | Gateway-generated authorization, dispatch, and evidence records | Backend systems may still need to verify enforcement proof |
| Cross-Domain Agent Interaction | High-Impact or Audit-Grade depending on effect | Multiple domains with explicit trust relationships | Domain boundary or receiving-side enforcement point | Cross-domain evidence correlation and authority verification | Trust, delegation, revocation, and evidence semantics may differ across domains |
| Backend Relying Party Verification | High-Impact and Audit-Grade | Backend system verifies enforcement proof before effect | Dispatcher plus backend relying-party checks | Backend-correlated evidence of actual system effect | Verification must be consistently implemented across backend entry points |
| Audit-Grade Separated TCB | Audit-Grade | Separated ADP, TDE, backend, evidence pipeline, and evidence store | Dedicated enforcement component outside model-controlled paths | Independently generated or corroborated, tamper-evident, action-bound evidence | Higher implementation and operational complexity |

These patterns are capability patterns, not product categories.

AAEF does not require a specific agent framework, orchestration library, policy engine, cloud provider, or SIEM product.

The important distinction is whether the implementation can demonstrate that model-generated output cannot by itself authorize, dispatch, execute, or self-attest a high-impact action.
